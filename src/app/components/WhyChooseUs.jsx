// Updated WhyChooseUs component with full Light & Dark Mode theme
"use client";
...
export default function WhyChooseUs() {
  ...
  return (
    <section
      id="why-choose"
      className="relative px-6 md:px-12 overflow-hidden bg-[#F5F7FA] dark:bg-[#0F1117] py-16"
    >
      ...
      <div className="text-center mb-12 relative z-10">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
          <h2 className="text-3xl md:text-4xl font-bold text-[#4A6CF7] flex items-center justify-center gap-3">
          ...
          <p className="text-[#4A5568] dark:text-[#A0AEC3] mt-3 text-lg">We make your growth faster, smarter, and effortless.</p>
        </motion.div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 relative z-10">
        {items.map((item, index) => (
          <motion.div
            ...
            className="bg-white dark:bg-[#1A1F2B] rounded-2xl p-6 shadow-lg text-center flex flex-col items-center border border-[#4A6CF7]/20"
          >
            <div className="w-20 h-20 mb-4 relative rounded-full overflow-hidden border border-[#4A6CF7]/30 shadow-md">
              <Image src={item.img} alt={item.title} fill className="object-cover" />
            </div>

            <h3 className="text-lg font-semibold text-[#4A6CF7] mb-2">{item.title}</h3>

            <p className="text-[#4A5568] dark:text-[#A0AEC3] text-sm">{item.desc}</p>
          </motion.div>
        ))}
      </div>
    </section>

...
import { Trash2, Lock, Unlock, Save, UserCircle2, Mail, Shield, Calendar, ImageOff } from "lucide-react";
...
export default function EditUserPage({ user }) {
  ...
  const handleChange = async (e) => {
    ...
    if (["username", "email"].includes(name)) {
      ...
      try {
        ...
        const isOwnValue = user[name]?.toLowerCase() === value?.toLowerCase();
  ...
  const toggleFreeze = async (freeze) => {
    ...
    try {
    const res =  user.frozen? await FreezeUser(user._id,false):  await FreezeUser(user._id,true)
      if (!res.success) throw new Error("Failed to update user status");
      setForm((prev) => ({ ...prev, frozen: freeze }));
      setSuccess(freeze ? "User account frozen successfully." : "User account reactivated.");
    } catch (err) {
      console.error(err);
    ...
    <div className="min-h-screen bg-gradient-to-b from-[#0b0b0c] via-[#0d0d10] to-[#141414] text-gray-100 p-6 sm:p-10">
      ...
      <div className="flex flex-col sm:flex-row items-center sm:items-start gap-6 mb-10 bg-[#151515]/80 border border-yellow-500/20 rounded-2xl p-6 shadow-[0_0_15px_rgba(255,215,0,0.05)]">
        ...
        <div className="flex-1 text-center sm:text-left">
          <h2 className="text-2xl font-semibold text-yellow-400">{form.username || "Unnamed User"}</h2>
          ...
          <p className="text-gray-400 flex items-center justify-center sm:justify-start gap-2 mt-1">
            <Shield size={16} /> Role:{" "}
            <span className="capitalize text-yellow-300">{form.role || "user"}</span>
          </p>
          <p className="text-gray-400 flex items-center justify-center sm:justify-start gap-2 mt-1">
        ...
        <input
          ...
          value={
            typeof value === "boolean"
              ? value.toString() // ✅ converts both true and false
              : value || ""
          }
          onChange={handleChange}

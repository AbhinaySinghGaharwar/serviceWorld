...
export default function ClientPage({ panels }) {
  ...
  // ✅ NEW modal states (injected only, no other logic touched)
  ...
  const saveStatus = () => {
    alert("Selected Status: " + selectedStatus); // ✅ Only alert, you will add API later
  ...
  return (
    ...
    {/* ✅ MODAL (embedded here, not separate component) */}
    {statusModalOpen && currentPanel && (
      <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center px-4 z-50">
        <div className="bg-white dark:bg-[#1A1C1F] border border-gray-300 dark:border-gray-700 rounded-xl w-full max-w-sm p-5 relative animate-fadeIn">
          ...
          <button
            onClick={saveStatus}
            className="mt-4 w-full py-2 bg-gray-800 text-white font-bold rounded-lg hover:bg-gray-700 text-sm"
          >
            Save Status
          </button>
